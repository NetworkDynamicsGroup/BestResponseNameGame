...
filename="best_response_name_game_output.csv"
...
### SET UP NETWORK GENERATOR
### THIS EXAMPLE USES A SIMPLE FULL NETWORK
### BUT ANY OTHER NETWORK AND SET OF PARAMETERS CAN BE
### INTRODUCED HERE.
generate_network = function(N) {
  g = graph.full(N)
}


### CYCLE THRUGH trials REPETITIONS
for(x in 1:trials) {
  print(x)
  ### CYCLE THROUGH DIFFERENT THRESHOLDS
  for(pA in 0.5) {
    
    ### CYCLE THROUGH DIFFERENT MEMORY LENGTHS
    for(maxmem in c(12)) {
      
      ### CYCLE THROUGH DIFFERENT POPULATION SIZES
      for(N in 1000) {
        
        ### THIS ALLOWS US TO USE ONE SIMLUATION
        ### TO EXAMINE THE PERFORMANCE AT VARIOUS POINTS IN TIME
        ### WITHOUT RECORDING EVERY SINGLE TIME POINT.
        checkt = c(N*1000)
